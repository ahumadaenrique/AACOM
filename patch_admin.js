...
const functions = \
  ...
  const handleCreateSchedule = async (e) => {
    ...
    if (res.success) {
      setPushStatus("Notificación programada con éxito!");
      setPushMessage("");
  ...
  const handleDeleteSchedule = async (id) => {
    ...
    if (res.success) {
      setPushStatus("Notificación borrada.");
      const schedRes = await getScheduledPushes();
      if (Array.isArray(schedRes)) setScheduledPushes(schedRes);
    } else {
      setPushStatus("Error: " + res.message);
...
code = code.replace('  const handleSendPush = async (e: React.FormEvent) => {', functions);
              ...
              <Card className="border shadow-sm">
                ...
                <CardContent className="pt-6 space-y-4">
                  <div className="flex items-center justify-between p-3 border rounded-xl">
                    ...
                    <Button variant={pushPointsEnabled ? "default" : "outline"} onClick={handleTogglePointsSetting}>
                  ...
                  <div className="flex items-center justify-between p-3 border rounded-xl">
                    ...
                    <Button variant={pushPlanningEnabled ? "default" : "outline"} onClick={handleTogglePlanningSetting}>
...
code = code.replace(/<div className="max-w-xl mx-auto">[\s\S]*?<form onSubmit=\{handleSendPush\} className="space-y-4">/, uiToggles);
...
code = code.replace(/<Button[\s\S]*?Disparar Alerta a Tel[^<]*?<\/Button>[\s]*?<\/form>/, pushButtons + \\n                  </div>\);
...
code = code.replace(/<\/Card>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?\)}/, tableSection + \\n            </div>\n          </div>\n        )}\);

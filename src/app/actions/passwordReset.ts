...
export async function requestPasswordReset(email: string) {
  try {
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: `${agencyName} <no-reply@aacomsoft.com>`,
        to: [user.email],
        subject: `Restablecer tu contraseña de ${agencyName}`,
        html: `
          <div style="font-family: system-ui, -apple-system, sans-serif; max-width: 500px; padding: 24px; border: 1px solid #e2e8f0; border-radius: 16px;">
            ...
            <p style="color: #334155; font-size: 14px; line-height: 1.5;">Hemos recibido una solicitud para restablecer la contraseña de tu cuenta en ${agencyName}. Haz clic en el botón de abajo para ingresar una nueva contraseña (este enlace expira en 1 hora):</p>
